...
public interface vueRepository extends CrudRepository<vue,Integer> {
    ...
    List<vue> All();
    @Query(value = "SELECT CONCAT('Nom client(e) : ' ,c.prenom , ' reserve la chambre ', ch.name ,' au prix de ', ch.prix, ' USD par jour, pour la periode allant du ', r.debut,' au ', r.fin,' faisant donc ', (SELECT DATEDIFF(r.fin,r.debut)),' jours. Le montant Total a payer est de ', r.montant*(SELECT DATEDIFF(r.fin,r.debut))) FROM `reservations` AS r INNER JOIN clients AS c ON r.idclient=c.id INNER JOIN chambres AS ch ON r.idchambre=ch.id WHERE r.id=?;",nativeQuery = true)
